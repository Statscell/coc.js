...
exports.handleClanChanges = exports.handlePlayerChanges = void 0;
function handlePlayerChanges(client, oldData, newData) {
    ...
    if (client.events.includes('PLAYER_ACHIEVEMENT_UPDATE')) {
        ...
        for (const ach of newData.achievements) {
            ...
            if (oldValue !== ach.value)
                achievements.push({ name: ach.name, previous: oldAch, current: ach });
        }
        if (achievements.length)
            client.emit('playerAchievementUpdate', achievements);
    }
    if (client.events.includes('PLAYER_TROOP_UPDATE')) {
        ...
        for (const hero of newData.heroes) {
            ...
            if (oldLvl !== hero.level)
                heroes.push({ name: hero.name, oldLevel: oldLvl, newLevel: hero.level, maxLevel: hero.maxLevel, village: hero.village });
        }
        const spells = [];
        for (const spell of newData.spells) {
            ...
            if (oldLvl !== spell.level)
                spells.push({ name: spell.name, oldLevel: oldLvl, newLevel: spell.level, maxLevel: spell.maxLevel, village: spell.village });
        }
        const troops = [];
        for (const troop of newData.troops) {
            ...
            if (oldLvl !== troop.level)
                troops.push({ name: troop.name, oldLevel: oldLvl, newLevel: troop.level, maxLevel: troop.maxLevel, village: troop.village });
        }
        const output = {};
        const keys = [];
        if (heroes.length) {
        ...
        if (keys.length)
            client.emit('playerTroopUpdate', output);
    }
}
exports.handlePlayerChanges = handlePlayerChanges;
function handleClanChanges(client, oldData, newData) {
...
exports.handleClanChanges = handleClanChanges;
